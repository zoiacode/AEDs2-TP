//package TP6.FilaDeRegistrosJava;
...
class Pilha {
    ...
    public void mostrar() {
        int idx = 0;
        for (No p = topo; p != null; p = p.prox, idx++) {
            System.out.println("[" + idx + "] " + p.game.getName());
        }
    }

    public void mostrarDaBase() {
        java.util.Deque<Game> aux = new java.util.ArrayDeque<>();
        for (No p = topo; p != null; p = p.prox)
            aux.push(p.game);
        int i = 0;
        while (!aux.isEmpty()) {
            System.out.println("[" + i++ + "] " + aux.removeLast().getName());
        }
    }
}
