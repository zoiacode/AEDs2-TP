...
class TabelaHashReserva {
	private Game[] tabela;
	private final int tamPrincipal = 21;
	private final int tamReserva = 9;
	private final int tamTotal = tamPrincipal + tamReserva;
	private int posReserva; 
	private int comparacoes;

	public TabelaHashReserva() {
		tabela = new Game[tamTotal];
		posReserva = tamPrincipal; 
		comparacoes = 0;
	}
	...
	public void inserir(Game g) {
		...
		if (tabela[pos] == null) {
			...
			if (tabela[pos].name.equals(g.name)) {
				tabela[pos] = g;
				return;
			}
			if (posReserva < tamTotal) {
				tabela[posReserva++] = g;
			}
		}
	}
	...
	public boolean pesquisarComImpressao(String nome) {
		...
		for (int i = tamPrincipal; i < posReserva; i++) {
			comparacoes++;
			if (tabela[i] != null && tabela[i].name.equals(nome)) {
				System.out.println(nome + ":  (Posicao: " + i + ") SIM");
				return true;
			}
		}
...
public class Main {
	public static void main(String[] args) {
		...
		TabelaHashReserva tabela = new TabelaHashReserva();
